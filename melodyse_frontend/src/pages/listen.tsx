...
export default function Listen({ track_list, artists_list }: any) {
  ...
  useEffect(() => {
    setLoading(true);
    axios
      .get(`${process.env.SITE_URL}/gettracks?page=${activeSection}`, {
  ...
  const search = () => {
    if (searchValue.length > 0) {
      ...
      axios
        .get(`${process.env.SITE_URL}/search?q=${searchValue}`)
        .then((res) => {
          setSearchLoading(false);
      ...
      <div className={styles.container}>
        <div className={styles.div1}>
          ...
          <div className={styles.sections} ref={container}>
            ...
            {searchRes &&
              searchRes.tracks.map((track) => <TrackCard track={track} />)}
          </div>
        </div>
        <div className={styles.div2}>
          ...
          <div className={styles.div2Content}>
            <div
              className={
                searchValue.length > 0 ? styles.artistsBottom : styles.artists
              }
            >
              {!searchLoading ? (
                !searchRes && (
              ...
              {searchRes &&
                searchRes.artists.map((artist) => <ArtistCard data={artist} />)}
            </div>
            <div
              className={
                searchValue.length > 0 ? styles.searchTop : styles.search
              }
            >
              <input
                placeholder="Search"
...
export async function getServerSideProps(context) {
  ...
  let artists = {};
  await axios
    .get(`${process.env.SERVER_SITE_URL}/gettracks?page=1`, {
      headers: {
    ...
    .catch((err) => console.log(err));
  await axios
    .get(`${process.env.SERVER_SITE_URL}/getartists`)
