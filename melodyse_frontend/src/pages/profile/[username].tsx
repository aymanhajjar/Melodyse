...
export default function Profile({ data }: any) {
  ...
  const pictureChange = (e) => {
    ...
    axios
      .post(`${process.env.SITE_URL}/change-pic`, formdata, {
  ...
  const handleKeyDownName = (e) => {
    if (e.key === "Enter") {
      if (nameVal.length > 0) {
        ...
        axios
          .post(`${process.env.SITE_URL}/change-name`, formdata, {
            withCredentials: true,
          })
          .then((res) => {
            setNameEdit(false);
    ...
    if (e.key === "Enter") {
      if (usernameVal.length > 0) {
        if (usernameVal == data.username) {
          ...
          axios
            .post(`${process.env.SITE_URL}/change-username`, formdata, {
              withCredentials: true,
            })
            .then((res) => {
              setUsernameEdit(false);
  ...
  const handleKeyDownDescription = (e) => {
    if (e.key === "Enter") {
      ...
      axios
        .post(`${process.env.SITE_URL}/change-description`, formdata, {
          withCredentials: true,
        })
        .then((res) => {
  ...
  const addRemoveFriendReq = (action) => {
    ...
    axios
      .post(`${process.env.SITE_URL}/request-friend`, formdata, {
        withCredentials: true,
      })
      .then((res) => {
  ...
  const removeFriend = () => {
    ...
    formdata.append("username", data.username);
    axios
      .post(`${process.env.SITE_URL}/remove-friend`, formdata, {
        withCredentials: true,
      })
      .then((res) => {
      ...
      <div className={styles.container}>
        <div className={styles.div1}>
          <div className={styles.profileDiv}>
            <img
              src={process.env.SITE_URL + data.picture}
            ...
            {data.can_edit && (
              ...
              >
                {picLoading ? (
                  <img
                    className={styles.picLoading}
                    src="/loading-melodyse.gif"
                  />
                ) : (
                  <img src="/icons/camera.png" className={styles.camera} />
                )}
                <input
                  id="profile-upload"
                  type="file"
          ...
          <div className={styles.info}>
            ...
            <div className={styles.username}>
              {usernameEdit ? (
                <input
                  className={
                    usernameError ? styles.usernameError : styles.usernameField
                  }
                  value={usernameVal}
              ...
              {descriptionEdit ? (
                ...
                <>
                  <p>
                    {data.description
                      ? data.description
                      : data.can_edit && "Add a description..."}
                  </p>
                  {data.can_edit && (
                    <img
                      className={styles.editPenDetails}
                      src="/icons/edit.png"
            ...
            <ProfileActionButton
              ...
              submit={
                isFriend
                  ? removeFriend
                  : requested
                  ? () => addRemoveFriendReq("remove")
                  : () => addRemoveFriendReq("add")
              }
            />
          )}
        </div>
        ...
        <div className={styles.div2}>
          <div className={styles.skillsContainer}>
            ...
            <div className={styles.skills}>
              {data.skills.length > 0
                ? data.skills.map((skill) => <UserSkillCard skill={skill} />)
                : "No Skills To Show"}
            </div>
          </div>
          ...
          <div className={styles.favorite}>
            ...
            <div className={styles.artists}>
              {data.favorite_songs.length > 0
                ? data.favorite_songs.map((song) => <Song data={song} />)
                : "No Favorite Songs"}
            </div>
          </div>
          ...
          <div className={styles.favorite}>
            ...
            <div className={styles.artists}>
              {data.completed_projects.length > 0
                ? data.completed_projects.map((prj) => (
                    <div className={styles.project}>{prj.title}</div>
                  ))
        ...
        <div className={styles.div3}>
          {!data.can_edit ? (
            <div className={styles.actions}>
              <ProfileActionButton
                name="Hire"
                pic="/icons/briefcase.png"
                color="#47E5BC"
                submit={() => setHireOpen(true)}
              />

              <ProfileActionButton
                name="Collab"
                pic="/icons/handshake.png"
                color="#FFE8D1"
                submit={() => setCollabOpen(true)}
              />
              ...
              {status && (
                <span className={styles.inviteSent}>Invite Sent!</span>
              )}
            </div>
          ) : (
            <span className={styles.editText}>
              This is how others see your profile. Hover to edit.
            </span>
          )}
          <div className={styles.additionalInfo}>
            ...
            <div className={styles.stats}>
              ...
              <span>
                {data.total_likes ? data.total_likes : 0} likes received
              </span>
            </div>
...
export async function getServerSideProps(context) {
  ...
  await axios
    .get(`${process.env.SERVER_SITE_URL}/profile/${username}`, {
      headers: {
