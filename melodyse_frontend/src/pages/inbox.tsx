...
export default function Inbox({ inbox, userData, chatOpened, username }: any) {
  ...
  useEffect(() => {
    if (router.query.username) {
      axios
        .get(
          `${process.env.SITE_URL}/getchat?username=${router.query.username}&&page=1`,
          {
  ...
  useEffect(() => {
    if (searchVal.length > 0) {
      axios
        .get(`${process.env.SITE_URL}/searchfriends?q=${searchVal}`, {
          withCredentials: true,
        })
  ...
  const selectFriend = (username) => {
    axios
      .get(`${process.env.SITE_URL}/getchat?username=${username}&&page=1`, {
        withCredentials: true,
      })
      .then((res) => {
        setSelectedChat(res.data.chat_id);
  ...
  useEffect(() => {
    ...
    if (selectedChat >= 0) {
      setLoadingMsg(true);
      axios
        .get(`${process.env.SITE_URL}/getchat?id=${selectedChat}&&page=1`, {
          withCredentials: true,
        })
        .then((res) => {
          setMessages(res.data.messages);
  ...
  const handleScroll = () => {
    ...
    if (loadingMore) {
      ...
      if (loadingMoreRect.top >= containerRect.top && hasMore) {
        ...
        axios
          .get(
            `${process.env.SITE_URL}/getchat?id=${selectedChat}&&page=${
              page + 1
            }`,
            {
              withCredentials: true,
    ...
    <>
      ...
      <div className={styles.container}>
        ...
        <div className={styles.chat}>
          {selectedChat < 0 && (
            <span className={styles.empty}>Select a conversation</span>
          )}
...
export async function getServerSideProps(context) {
  let inbox = {};
  await axios
    .get(`${process.env.SERVER_SITE_URL}/getchats`, {
      headers: {
