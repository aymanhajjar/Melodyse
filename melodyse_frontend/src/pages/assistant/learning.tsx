...
export default function Learning({ skills }: any) {
  ...
  useEffect(() => {
    if (selectValue) {
      ...
      setTips();
      axios
        .get(`${process.env.SITE_URL}/gettips?skill=${selectValue}`, {
  ...
  const suggestSongs = () => {
    ...
    axios
      .get(
        `${process.env.SITE_URL}/songstolearn?skill=${selectValue}&&with_interests=${withInterests}`,
        {
          withCredentials: true,
        }
      )
  ...
  const suggestResources = () => {
    ...
    axios
      .get(
        `${process.env.SITE_URL}/suggestresources?skill=${selectValue}&&with_interests=${withInterests}`,
        {
          withCredentials: true,
        }
      )
  ...
  const explainMusic = () => {
    ...
    axios
      .get(
        `${process.env.SITE_URL}/explainmusic?skill=${selectValue}&&with_interests=${withInterests}`,
        {
          withCredentials: true,
        }
      )
      ...
      <div className={styles.container}>
        ...
        <div className={styles.contents}>
          ...
          <div className={styles.div2}>
            <AIActionButtonWide
              name="Suggest songs to learn"
              pic="/icons/question.png"
              submit={suggestSongs}
              loading={songsLoading}
            />
            <AIActionButtonWide
              name="Suggest resources to use"
              pic="/icons/book.png"
              submit={suggestResources}
              loading={resourcesLoading}
            />
            <AIActionButtonWide
              name="Explain Music Theory"
              pic="/icons/theory.png"
              submit={explainMusic}
        ...
        {response && (
          <PopupResponse response={response} close={() => setResponse()} />
        )}
      </div>
    </>
  );
...
Learning.getInitialProps = async (ctx) => {
  ...
  await axios
    ...
    .then((res) => {
      cleanData = res.data.filter(
        (skill) =>
          skill.name != "Record Managing" &&
          skill.name != "Music Listening" &&
          skill.name != "Composing"
      );
    })
    .catch((err) => console.error(err));
