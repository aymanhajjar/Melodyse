...
export default function FavoriteArtists(props: any) {
  ...
  const getFavoriteArtists = () => {
    axios
      .get(`${process.env.SITE_URL}/getfavoriteartists`, {
  ...
  const submit = () => {
    ...
    axios
      .post(`${process.env.SITE_URL}/addartists`, data, {
        withCredentials: true,
      })
      .then((res) => {
        setLoading(false);
  ...
  const search = () => {
    if (searchVal.length > 0) {
      ...
      axios
        .get(
          `https://api.spotify.com/v1/search?q=${searchVal}&&type=artist&&limit=21`,
          {
            headers: {
              "x-csrftoken": null,
              Authorization: `Bearer ${spotifyToken}`,
  ...
  const remove = (artist) => {
    setChosenArtists(
      chosenArtists.filter((item) => item["id"] !== artist["id"])
    );
    setChosenIDs(chosenIDs.filter((id) => id !== artist.id));
  };

  const addRemoveArtist = (artist) => {
    chosenIDs.includes(artist.id)
      ? setChosenArtists(
          chosenArtists.filter((item) => item["id"] !== artist["id"])
        )
      : chosenArtists.length <= 9 &&
                ...
                if (chosenIDs.includes(artist.id)) {
                  return (
                    <Artist
                      ...
                      index={index}
                      checked={true}
                      addRemove={() => addRemoveArtist(artist)}
                    />
                  );
                } else {
                  return (
