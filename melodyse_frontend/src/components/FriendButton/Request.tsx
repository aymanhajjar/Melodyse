...
export default function Request({ type, request }) {
  ...
  const respond = (action) => {
    ...
    axios
      .post(`${process.env.SITE_URL}/respond-request`, data, {
      ...
      {type == "friend" ? (
        <div className={request.is_seen ? styles.reqRead : styles.reqUnread}>
          <img
            ...
            src={process.env.SITE_URL + request.sender_picture}
          />

          <div className={styles.details}>
            <h3
              onClick={() => router.push(`/profile/${request.sender_username}`)}
            >
              {request.sender_name}{" "}
            ...
            {accepted === null ? (
              ...
              <h6 className={styles.accepted}>
                {" "}
                <img src={"/icons/check.png"} /> accepted
              </h6>
            ) : (
              <h6 className={styles.rejected}>
                <img src={"/icons/x.png"} />
                rejected
              </h6>
            )}
        ...
        <div className={request.is_seen ? styles.reqRead : styles.reqUnread}>
          ...
          <div className={styles.details}>
            <h3 onClick={() => setDetailsOpen(true)}>
              {request.sender_name}{" "}
              <span>
                {" "}
                {request.is_collab
                  ? "has invited you to a collaboration project!"
                  : "wants to hire you on a project!"}
              </span>
            </h3>
            {!request.is_collab && (
              <span>
                <b>Offered amount: </b>${request.offered_amount}
              </span>
            )}
            <span
        ...
        <div className={request.is_seen ? styles.reqRead : styles.reqUnread}>
          ...
          <div className={styles.details}>
            <h3 onClick={() => setDetailsOpen(true)}>
              ...
              <span>
                {" "}
                wants to join you on your project {request.project_name}
              </span>
            </h3>
            <span
              className={styles.moredetails}
