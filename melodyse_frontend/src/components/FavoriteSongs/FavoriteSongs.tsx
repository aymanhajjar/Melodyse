...
export default function FavoriteSongs(props: any) {
  ...
  const getSongs = () => {
    axios
      .get(
        `https://api.spotify.com/v1/recommendations?seed_genres=pop,country&&min_popularity=80&&limit=14`,
        {
          headers: {
            "x-csrftoken": null,
  ...
  const getFavoriteSongs = () => {
    axios
      .get(`${process.env.SITE_URL}/getfavoritesongs`, {
        withCredentials: true,
      })
      .then((res) => {
        res.data && setChosenSongs(res.data);
        const ids = [];
        res.data &&
  ...
  const submit = () => {
    ...
    axios
      .post(`${process.env.SITE_URL}/addsongs`, data, {
        withCredentials: true,
      })
      .then((res) => {
        setLoading(false);
  ...
  const search = () => {
    if (searchVal.length > 0) {
      ...
      axios
        .get(
          `https://api.spotify.com/v1/search?q=${searchVal}&&type=track&&limit=14`,
          {
            headers: {
              "x-csrftoken": null,
              Authorization: `Bearer ${spotifyToken}`,
