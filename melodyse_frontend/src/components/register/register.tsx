...
export default function Register(props: any) {
  ...
  const handleSubmit = () => {
    if (validateForm()) {
      ...
      axios
        .post(`${process.env.SITE_URL}/register`, data, {
          withCredentials: true,
        })
        .then((res) => {
          setLoading(false);
        ...
        .catch((err) => {
          ...
          if (err.response.data == "failed to validate password") {
            setErrorMessage(
              "Failed to validate password. Please make sure your password meets the requirements."
            );
          }
          ...
          if (err.response.data == "email already exists") {
            setErrorMessage(
              "An account with this email address already exists. Please choose another one, or log in instead."
            );
          }
          console.error(err);
        });
  ...
  return (
    <div className={active ? styles.active : styles.hidden}>
      ...
      <div className={styles.fullName}>
        <input
          className={firstNameError ? styles.inputErrorName : styles.inputName}
      ...
      <button type="button" className={styles.loginBtn} onClick={handleSubmit}>
        {loading ? <img src="/loadinggif.gif" /> : "SIGN UP"}
      </button>
      <span>
        Already have an account? <a onClick={() => changeForm()}>Log In</a>
      </span>
