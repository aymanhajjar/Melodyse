...
export default function ChosenSkill({name, remove, rating, updateRating}: any) {
    ...
    return(
        <div className={styles.artistCard}>
            ...
            {[...Array(5)].map((_, index) => (
                <img
                key={index}
                src={index < chosenRating ? '/icons/starEnabled.png' : '/icons/starDisabled.png'}
                onClick={() => index < chosenRating ? decreaseRating(index) : increaseRating(index)}
              />
            ))}
